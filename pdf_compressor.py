...
def _subprocess_rc(cmd, timeout=30):
    import subprocess
    try:
        # Guard against None executable
        if isinstance(cmd, (list, tuple)) and (not cmd or not cmd[0]):
            return 1
...
class DependencyChecker:
    ...
    def resolve_cmd(name: str) -> str | None:
        """Resolve a command by preferring bundled bin/, then PATH, then common locations.

        Returns absolute path or None if not found.
        """
        # Bundled alongside app (PyInstaller or repo bin/)
        bundled = get_resource_path(f"bin/{name}")
        ...
        try:
            ...
            pass
        # Common macOS/Linux locations (Homebrew and legacy)
        candidates = [
    ...
    @staticmethod
    def check_poppler():
        exe = DependencyChecker.resolve_cmd('pdftoppm')
        return _subprocess_rc([exe, '-h']) == 0 if exe else False

    @staticmethod
    def check_pdfimages():
        exe = DependencyChecker.resolve_cmd('pdfimages')
        if not exe:
            return False
        rc = _subprocess_rc([exe, '-v'])
        return rc in (0, 1)

    @staticmethod
    def check_qpdf():
        exe = DependencyChecker.resolve_cmd('qpdf')
        return _subprocess_rc([exe, '--version']) == 0 if exe else False

    @staticmethod
    def check_ghostscript():
        exe = DependencyChecker.resolve_cmd('gs')
        return _subprocess_rc([exe, '-v']) == 0 if exe else False
...
class PDFAnalyzer:
    ...
    def _probe_with_pdfimages(pdf_path, sample_limit=10):
        ...
        try:
            ...
            if not exe:
                return {'has_bilevel': False, 'image_dominant_pages': 0}
            r = subprocess.run([exe, '-list', pdf_path],
...
class PDFCompressorWorker(QThread):
    ...
    def compress_with_ghostscript(self):
        ...
        gs_exe = DependencyChecker.resolve_cmd('gs')
        if not gs_exe:
            raise Exception("Ghostscript not found. Install it or adjust PATH.")
        cmd = [
            gs_exe, '-sDEVICE=pdfwrite', '-dCompatibilityLevel=1.6',
    ...
    def _maybe_linearize(self, out_path):
        try:
            ...
            if not exe:
                return
            r = subprocess.run([exe, '--linearize', out_path, tmp_lin],
